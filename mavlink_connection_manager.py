...
# Initialize centralized logger for connection management
logger = MAVLinkLogger("ConnectionManager")
...
def connect_mavlink(drone_state, drone_state_lock, mavlink_connection_string_config):
    ...
    logger.info("Creating new MAVLink connection", connection_string=mavlink_connection_string_config)
    ...
    try:
    # Creating MAVLink connection with simplified parameters
    ...
    # MAVLink connection object created
        logger.info("Waiting for heartbeat to confirm connection")
        ...
        if not msg:
            logger.warning("No heartbeat received within timeout period, connection not confirmed")
            mavlink_connection_instance.close()
            mavlink_connection_instance = None
            return False
        
        logger.info("Received initial heartbeat, connection confirmed", src_system=msg.get_srcSystem())
        ...
        # Drone mode and armed status will be logged via heartbeat processing
        ...
        logger.debug("Set target system and component", target_system=mavlink_connection_instance.target_system, target_component=mavlink_connection_instance.target_component)
        ...
        logger.error("MAVLink connection error", error=str(e))
...
def request_data_streams(req_rate_hz_config, home_position_is_known):
    ...
    if not mavlink_connection_instance or not mavlink_connection_instance.target_system:
        logger.warning("No MAVLink connection or target system to request streams")
        return

    logger.info("Requesting data streams", rate_hz=req_rate_hz_config)
    ...
    for msg_id, msg_name in messages_to_request:
        try:
            logger.debug("Requesting message stream", msg_name=msg_name, msg_id=msg_id, rate_hz=req_rate_hz_config, interval_us=interval_us)
            ...
            logger.error("Error requesting message stream", msg_name=msg_name, error=str(e))

    logger.info("Data stream requests completed", rate_hz=req_rate_hz_config)
...
def check_pending_command_timeouts(sio, command_ack_timeout_config, log_function):
    ...
    for cmd_id, value in list(pending_commands_instance.items()): # Iterate over a copy
        ...
        if now - timestamp > command_ack_timeout_config:
            ...
            logger.warning("Command timeout detected", command_id=cmd_id, timeout_seconds=command_ack_timeout_config)
            if log_function:
...
def mavlink_receive_loop_runner(
    ...
    logger.info("MAVLink receive loop thread started")
    while True:
        ...
        if not mavlink_connection_instance: # Primary check: is there a connection object at all?
            ...
            if not connect_mavlink(drone_state, drone_state_lock, connection_string_to_use):
                logger.warning("MAVLink connection object creation failed, retrying in 5s")
            ...
            logger.info("Connection object created, waiting for first heartbeat")
            ...
            while time.time() - wait_for_heartbeat_start_time < 5: # 5-second timeout
                ...
                if msg:
                    logger.info("First heartbeat received, connection confirmed")
            ...
            if not initial_heartbeat_received:
                logger.warning("No heartbeat received within 5 seconds, closing connection and retrying")
                mavlink_connection_instance.close()
                mavlink_connection_instance = None
                gevent.sleep(5)
                continue
        ...
        if last_heartbeat_time_instance != 0 and (time.time() - last_heartbeat_time_instance > heartbeat_timeout_config):
            logger.warning("Heartbeat timeout, drone disconnected", timeout_seconds=heartbeat_timeout_config)
            with drone_state_lock:
                drone_state['connected'] = False
        ...
        try:
            ...
            while True:
                ...
                # Message reception logged at debug level if needed
                ...
                if msg_type == 'HEARTBEAT':
                    ...
                    logger.debug("Processing non-heartbeat message", msg_id=msg.get_msgId(), msg_type=msg_type)
                    handler = MAVLINK_MESSAGE_HANDLERS.get(msg_type)
                    if handler:
                        logger.debug("Found message handler", handler_name=handler.__name__, msg_type=msg_type)
                        try:
                            handler_kwargs = {
                                'msg': msg,
                            ...
                            logger.debug("Calling message handler", handler_name=handler.__name__, kwargs_keys=list(valid_kwargs.keys()))
                            handler_changed_state = handler(**valid_kwargs)
                            logger.debug("Message handler completed", handler_name=handler.__name__, msg_type=msg_type, state_changed=handler_changed_state)
                            if handler_changed_state:
                                drone_state_changed_iteration = True # Update flag if handler reported change
                        except Exception as e:
                            logger.error("Message handler error", msg_type=msg_type, handler_name=handler.__name__, error=str(e))
                            import traceback
                            traceback.print_exc() # Cascade: Add full traceback for errors
                    else:
                        logger.debug("No handler found for message", msg_id=msg.get_msgId(), msg_type=msg_type)
                        pass
            ...
            if messages_processed_this_cycle == 0:
                ...
                if current_time - mavlink_receive_loop_runner.last_message_debug_time > 5:  # Debug every 5 seconds
                    logger.debug("No messages received in polling iteration", timestamp=current_time)
            ...
            logger.error("Error in MAVLink receive loop, attempting to recover", error=str(e))
            # import traceback
...
def force_reconnect_with_new_address(new_connection_string, drone_state, drone_state_lock=None):
    ...
    try:
        logger.info("Force reconnection requested", new_connection_string=new_connection_string)
        ...
        if mavlink_connection_instance:
            logger.info("Closing existing MAVLink connection")
            try:
                mavlink_connection_instance.close()
            except Exception as e:
                logger.warning("Error closing existing connection", error=str(e))
            mavlink_connection_instance = None
        ...
        logger.info("Force reconnection setup complete", connection_string=new_connection_string)
        return True
        
    except Exception as e:
        logger.error("Error during force reconnection setup", error=str(e))
        return False
...
def force_disconnect(drone_state, drone_state_lock=None):
    ...
    try:
        logger.info("Force disconnection requested")
        ...
        logger.info("Force disconnection complete")
        return True
        
    except Exception as e:
        logger.error("Error during force disconnection", error=str(e))
        return False
