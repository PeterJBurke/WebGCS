...
from webgcs_logger import UILogger
...
# Initialize centralized logger for UI operations
logger = UILogger("SocketIOHandlers")
...
def _log_wrapper_for_caller_info(command_name, params=None, details=None, level="INFO"):
    ...
    if _log_command_action: # Check if the main logger from app.py is available
        ...
        logger.warning("Logger not initialized", level=level, command_name=command_name, params=params, details=details, caller_filename=caller_filename, caller_lineno=caller_lineno, caller_line_content=caller_line_content)
        ...
        if command not in [mavutil.mavlink.MAV_CMD_SET_MESSAGE_INTERVAL, mavutil.mavlink.MAV_CMD_REQUEST_MESSAGE]:
            ...
            if len(_pending_commands_dict) > 30:
                ...
                if oldest_cmd is not None:
                    logger.warning("Pending command limit reached, removing oldest", oldest_cmd=oldest_cmd)
...
def init_socketio_handlers(socketio_instance, app_context):
    ...
    def handle_send_command(data):
        ...
        if cmd == 'ARM':
            ...
            try:
                ...
                # Check current mode
                current_mode = _drone_state.get('mode', 'UNKNOWN')
                logger.debug("TAKEOFF: Checking if GUIDED mode change needed", current_mode=current_mode)
                
                # First, set to GUIDED mode if not already in GUIDED mode
                if current_mode != 'GUIDED':
                    logger.debug("TAKEOFF: Setting GUIDED mode first", current_mode=current_mode)
                    ...
                    logger.debug("TAKEOFF: Sending SET_MODE to GUIDED", mode_id=_AP_MODE_NAME_TO_ID['GUIDED'])
                    guided_success, guided_msg = _send_mavlink_command_handler(
                        mavutil.mavlink.MAV_CMD_DO_SET_MODE,
                        p1=mavutil.mavlink.MAV_MODE_FLAG_CUSTOM_MODE_ENABLED,
                        p2=_AP_MODE_NAME_TO_ID['GUIDED']
                    )
                    logger.debug("TAKEOFF: SET_MODE result", success=guided_success, msg=guided_msg)
                    ...
                    logger.debug("TAKEOFF: Already in GUIDED mode, sending takeoff command directly")
                    success, msg_send = _send_mavlink_command_handler(
                        mavutil.mavlink.MAV_CMD_NAV_TAKEOFF,
                        p7=alt  # param7 = altitude
                    )
            ...
            try:
                ...
                # Check current mode
                current_mode = _drone_state.get('mode', 'UNKNOWN')
                logger.debug("GOTO: Checking if GUIDED mode change needed", current_mode=current_mode)
                ...
                # First, set to GUIDED mode if not already in GUIDED mode
                if current_mode != 'GUIDED':
                    logger.debug("GOTO: Setting GUIDED mode first", current_mode=current_mode)
                    ...
                    # Send SET_MODE to GUIDED command first
                    logger.debug("GOTO: Sending SET_MODE to GUIDED", mode_id=_AP_MODE_NAME_TO_ID['GUIDED'])
                    guided_success, guided_msg = _send_mavlink_command_handler(
                        mavutil.mavlink.MAV_CMD_DO_SET_MODE,
                        p1=mavutil.mavlink.MAV_MODE_FLAG_CUSTOM_MODE_ENABLED,
                        p2=_AP_MODE_NAME_TO_ID['GUIDED']
                    )
                    logger.debug("GOTO: SET_MODE result", success=guided_success, msg=guided_msg)
                    ...
                    logger.debug("GOTO: Already in GUIDED mode, proceeding with GOTO command")
    ...
    def goto_location(sid, data):
        logger.debug("goto_location event received", sid=sid, data=data)
